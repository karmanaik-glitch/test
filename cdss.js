...
/* FIX #7: Guard against history.back() navigating out of SPA when there
   is no prior in-app history entry (e.g. user loaded the app via direct URL). */
function closeWardMode() {
  if (isWardMode) {
    if (history.state && history.state.wardOpen) {
      history.back();
    } else {
...
function generateADR() {
  ...
  navigator.clipboard.writeText(r)
    .then(() => { toast('Report copied & saved to timeline. Submit at pvpi.gov.in', 'ok', 5000); closeM('adr-modal'); })
    .catch(() => { toast('Report saved to patient timeline. Copy manually if needed.', 'warn', 6000); closeM('adr-modal'); });
}
...
async function genCarePlan() {
  ...
  if (!isNaN(age) && !isNaN(wt) && !isNaN(scr) && scr > 0) {
    ...
    if (!isNaN(ht) && ht > 0) {
      ...
      if (wt > 1.3 * ibwClamped && ibwClamped > 0) {
        dosingWt = ibwClamped + 0.4 * (wt - ibwClamped);
      }
    }
  ...
  /* FIX #10: Explicit renal note prevents the AI from using CrCl for CKD staging */
  payload.labs.renal_note = "The calculated_CrCl_CG above is a Cockcroft-Gault estimate for DRUG DOSING ONLY. " +
    "Do NOT use it to stage CKD (staging requires CKD-EPI eGFR, which is not provided). " +
    "If CKD staging is relevant, note it as 'CKD stage unknown — eGFR not available'.";
  ...
  try {
    ...
    saveWardData();
    /* FIX #5: _wardFormDirty cleared AFTER successful save but BEFORE AI call.
       If the AI fails, the patient data is committed; the form shows a retry
       button in the error state rather than relying on dirty-flag detection. */
    _wardFormDirty = false;
...
2. RENAL/HEPATIC: Use 'calculated_CrCl_CG' for drug dose adjustments. This is Cockcroft-Gault for DRUG DOSING ONLY — do NOT use it to stage CKD. See 'renal_note' in the payload.
...
Output ONLY valid JSON:
{
  ...
  try {
    ...
    if(mode === 'discharge') {
      html = `<div class="cdss-report" style="border-left:4px solid var(--ok);background:rgba(16,185,129,0.05);">
        ...
        <div style="width:100%;overflow-x:auto;margin-bottom:15px;border-radius:8px;border:1px solid var(--b);">
          ...
          ${(result.medications||[]).map(m=>`<tr><td><strong>${esc(m.name||'-')}</strong></td><td>${esc(m.purpose||'-')}</td><td>${esc(m.schedule||'-')}</td><td>${esc(m.warnings||'-')}</td></tr>`).join('')}</table>
        </div>
        <div style="margin-top:15px;"><div style="font-weight:700;color:var(--text);margin-bottom:8px;"><span class="ms sm">restaurant_menu</span> Diet &amp; Lifestyle</div>
          <ul style="font-size:0.85rem;margin-left:20px;margin-bottom:15px;">${(result.lifestyle_and_diet||[]).map(t=>`<li>${esc(t)}</li>`).join('')}</ul></div>
        <div style="margin-top:15px;padding-top:15px;border-top:1px solid var(--b);">
          ...
          <ul style="font-size:0.85rem;margin-left:20px;">${(result.when_to_call_doctor||[]).map(w=>`<li>${esc(w)}</li>`).join('')}</ul></div>
        ${aiDisclaimer}
      </div>`;
    } else {
      const sS = soap.S || {};
      const sO = soap.O || {};
      const sP = soap.P || {};
      const sContent = `<div style="margin-bottom:6px;"><strong>Chief Complaint:</strong> ${esc(sS.chief_complaint || 'Not provided.')}</div><div><strong>History:</strong> ${esc(sS.history || 'Not provided.')}</div>`;
      const therapyRows = Array.isArray(sO.current_therapy) && sO.current_therapy.length > 0
        ? sO.current_therapy.map(c=>`<tr><td><strong>${esc(c.drug||'-')}</strong></td><td>${esc(c.dose||'-')}</td><td>${esc(c.freq||'-')}</td><td>${esc(c.route||'-')}</td><td>${esc(c.indication||'-')}</td><td>${esc(c.moa||'-')}</td><td>${esc(c.monitoring||'-')}</td><td>${esc(c.side_effects||'-')}</td></tr>`).join('')
        : '<tr><td colspan="8" style="text-align:center;">No current therapy recorded.</td></tr>';
      const oContent = `<div style="margin-bottom:8px;"><strong>Vitals:</strong> ${esc(sO.vitals || 'Not provided.')}</div><div style="margin-bottom:10px;"><strong>Labs:</strong> ${esc(sO.labs || 'Not provided.')}</div>
        ...
        <div style="width:100%;overflow-x:auto;border-radius:8px;border:1px solid var(--b);">
          <table class="rep-tbl" style="min-width:860px;margin:0;"><tr><th>Drug</th><th>Dose</th><th>Freq</th><th>Route</th><th>Indication</th><th>MOA</th><th>Monitoring</th><th>Side Effects</th></tr>${therapyRows}</table></div>`;
      const correlationsHtml = Array.isArray(sA.clinical_correlations) && sA.clinical_correlations.length > 0
        ? `<div style="margin-bottom:10px;"><strong>Clinical Correlations:</strong><ul style="margin:5px 0 0 18px;">${sA.clinical_correlations.map(c=>`<li>${esc(c)}</li>`).join('')}</ul></div>` : '';
      const drpItems = Array.isArray(sA.drug_related_problems) && sA.drug_related_problems.length > 0
        ? sA.drug_related_problems.map(drp => {
            ...
            const pairBadge = isDDI && drp.drug_pair
              ? `<div style="display:inline-flex;align-items:center;gap:6px;margin:6px 0 2px;padding:4px 10px;background:rgba(239,68,68,0.1);border:1px solid rgba(239,68,68,0.35);border-radius:6px;font-size:0.78rem;font-weight:700;color:var(--danger);letter-spacing:0.01em;"><span style="font-size:13px;line-height:1;">\u26A1</span>${esc(drp.drug_pair)}</div>` : '';
            return `<div class="aalert ${badgeClass}" style="margin-bottom:8px;flex-direction:column;">
              <div style="font-weight:700;text-transform:uppercase;font-size:0.7rem;">${esc(drp.category||'Issue')} [${esc(drp.severity ? drp.severity.toUpperCase() : 'UNKNOWN')} RISK]</div>
              ${pairBadge}
      ...
      const aContent = correlationsHtml + `<div style="margin-top:8px;"><strong style="font-size:0.8rem;text-transform:uppercase;letter-spacing:0.05em;">Drug-Related Problems</strong><div style="margin-top:8px;">${drpItems}</div></div>`;
      ...
      const pContent = `<div style="margin-bottom:10px;"><strong>Pharmacist Interventions:</strong><ul style="margin:5px 0 0 18px;">${interventionsHtml}</ul></div>
        ${monitoringHtml ? `<div style="margin-bottom:10px;"><strong>Monitoring Plan:</strong><ul style="margin:5px 0 0 18px;">${monitoringHtml}</ul></div>` : ''}
        <div style="margin-bottom:10px;"><strong>Follow-up:</strong> ${esc(sP.follow_up || 'Not specified.')}</div>
        <div style="font-size:0.72rem;color:var(--muted);border-top:1px dashed var(--b);padding-top:8px;margin-top:4px;"><strong>References:</strong><br>${referencesHtml || 'None cited.'}</div>`;
      html = `<div class="cdss-report">
        <div class="rep-hdr"><span class="ms sm">description</span>
    ...
    const loaderEl = document.getElementById(loaderId);
    /* FIX #5: Error state shows a Retry button so clinician can re-run the AI
       without needing to re-enter the form (patient data was already saved). */
    if(loaderEl) loaderEl.innerHTML = `<div class="aalert dng">
      <div><span class="ms sm">error</span> <strong>Analysis Failed</strong><br>${esc(e.message || 'The AI returned an error.')}</div>
      <button class="btn-block sec" style="margin-top:12px;" onclick="callCDSSAI(null,'${mode}','${lang}')">
        <span class="ms sm">refresh</span> Retry AI Analysis
      </button>
    </div>`;
  }
}
...
function qDoseCheck(id) {
  const pt = wardPatients.find(p=>p.id===id);
  if(!pt) return;
  if(!Array.isArray(pt.currentTx) || pt.currentTx.length === 0) { toast('No active treatments to check.', 'warn'); return; }
...
<meta charset="UTF-8"><title>PharmAI SOAP Report \u2014 ${ptName}</title>
<style>
*,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
@page{size:A4;margin:15mm 12mm}
body{font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Arial,sans-serif;font-size:11px;color:#111;background:#fff;line-height:1.55}
.print-hdr{display:flex;justify-content:space-between;align-items:flex-start;padding-bottom:10px;border-bottom:2px solid #111;margin-bottom:18px}
...
.cdss-report{border:1px solid #ccc;border-radius:6px;padding:14px;margin-bottom:22px;page-break-inside:avoid;background:#fff;border-left:4px solid #333}
.rep-hdr{font-size:12px;font-weight:800;border-bottom:1.5px solid #111;padding-bottom:6px;margin-bottom:10px;color:#111}
table{width:100%;border-collapse:collapse;font-size:10px;margin:8px 0 12px;page-break-inside:auto}
th{background:#eee;color:#111;font-weight:700;padding:5px 7px;border:1px solid #aaa;text-align:left;-webkit-print-color-adjust:exact;print-color-adjust:exact}
td{padding:5px 7px;border:1px solid #ccc;color:#111;vertical-align:top}
tr:nth-child(even) td{background:#f9f9f9}
tr{page-break-inside:avoid}
.aalert{border:1px solid #ccc;background:#f5f5f5;padding:8px 10px;border-radius:6px;margin-bottom:6px;color:#111;display:flex;flex-direction:column;gap:2px;-webkit-print-color-adjust:exact;print-color-adjust:exact}
...
.print-footer{margin-top:20px;padding-top:8px;border-top:1px solid #ccc;font-size:9px;color:#888;display:flex;justify-content:space-between}
</style>
</head><body>
  <div class="print-hdr">
    <div class="print-hdr-left"><h1>PharmAI \u2014 Pharmacist SOAP Report</h1><div class="sub">${ptName} &nbsp;\u00B7&nbsp; Bed ${ptBed} &nbsp;\u00B7&nbsp; ${ptInfo}</div></div>
    <div class="print-hdr-right"><div><strong>Printed:</strong> ${printDate}</div><div><strong>Pharmacist:</strong> ${reporter}</div><div><strong>App:</strong> PharmAI Clinical Assistant</div></div>
  </div>
  ${reportHTML}
